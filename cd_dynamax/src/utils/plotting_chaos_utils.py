...
def _plot_time_series(t, states_true, states_learned, state_labels):
    ...
    for i, lbl in enumerate(state_labels):
        ...
        axs[i].plot(
            t, states_learned[:, i], label=f"Learned {lbl}", color="C1", alpha=0.7
        )
...
def _plot_phase_portraits(states_true, states_learned):
    ...
    for ax, (i, j) in zip(axs, pairs):
        ax.plot(
            states_true[:, i], states_true[:, j], label="True", color="C0", alpha=0.8
        )
        ax.plot(
            states_learned[:, i],
            states_learned[:, j],
...
def _plot_autocorr(states_true, states_learned, max_lag=500):
    ...
    for i in range(states_true.shape[1]):
        ...
        axs[i].plot(
            lags, autocorr(states_learned[:max_lag, i]), label="Learned", color="C1"
        )
...
def plot_correlation_dimension_comparison(
    ...
    plt.plot(
        log_eps_t, log_C_t, "o-", color="C0", label=f"{label_true} (D₂ ≈ {D2_true:.2f})"
    )
    ...
    plt.plot(
        ...
        "o-",
        color="C1",
        label=f"{label_learn} (D₂ ≈ {D2_learn:.2f})",
    )
    ...
    # Print results
    print("Estimated Correlation Dimension D2:")
...
def analyze_chaotic_dynamics(
    ...
    _plot_kde_comparison(
        states_true_long, states_long_learned, "KDE of states: True vs Learned"
    )
    _plot_kde_comparison_2d(
        states_true_long, states_long_learned, "KDE of states: True vs Learned"
    )
    ...
    print(
        f"Estimated Correlation Dimensions: True D2={D2_true:.3f}, Learned D2={D2_learn:.3f}"
    )
