...
from cd_dynamax.src.utils.simulation_utils import make_key_sequence
...
def generate_data_from_config(
    ...
    if not os.path.exists(data_config_filepath):
        raise FileNotFoundError(
            f"Configuration file '{data_config_filepath}' not found."
        )
    ...
    if overrides is not None:
        data_config = override_config(cfg=data_config, overrides=overrides)
    ...
    if data_save_file is None:
        ...
        if "data_saving" in data_config:
            ...
            print(
                f"DATA Warning: 'data_save_file' in config is set to={data_config['data_saving'].get('data_save_file')} \n we are using the provided data_save_file={data_save_file} instead."
            )
    ...
    if os.path.exists(data_save_file):
        print(
            f"NO DATA GENERATED: The file '{data_save_file}' already exists, data is loaded from the file."
        )
        ...
        if "data_generation" in data_config:
            ...
            num_timesteps, t_emissions = generate_t_emissions(
                t0, t1, num_samples, irregular_samples=irregular_samples, key=next(keys)
            )

            # Transition type for data generation
            transition_type = data_gen_config.get(
            ...
            raise ValueError(
                "Configuration file must contain 'data_generation' section."
            )
        ...
        if os.path.exists(data_save_file):
            print(
                f"Warning: The file '{data_save_file}' already exists. It will be overwritten."
            )
...
if __name__ == "__main__":
    ...
    generated_data, data_key = generate_data_from_config(
        data_config_file=sys.argv[1], data_save_file=sys.argv[2]
    )
