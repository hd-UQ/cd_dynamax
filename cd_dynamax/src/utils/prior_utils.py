...
# CD-Dynamax imports
from cd_dynamax import Prior, ParamsCDLGSSM
...
class CDLGSSM_WeightPrior(Prior):
    ...
    def __init__(self, params: ParamsCDLGSSM, **kwargs):
    ...
    def log_prob(self, x):
        ...
        log_prob = self.dynamic_weights_prior.log_prob(dynamic_weights_samples)
...
class CDLGSSM_DynamicsPrior(Prior):
    ...
    def sample(self, key, M):
        ...
        diffusion_coefficient = (
            self.dynamics_diffusion_coefficient_prior.sample(
                (M, self.state_dim),
                seed=key,
            )[..., None]
            * jnp.eye(self.state_dim)[None, ...]
        )  # Expand to the diagonal of the full L_t matrix
    ...
    def log_prob(self, x):
        ...
        if x.dynamics.weights.ndim > len(self.dynamic_weights_shape):
            ...
            # Flatten the weights, keeping the first dimension for M
            dynamic_weights_samples = x.dynamics.weights.reshape((1, -1))

        # Compute logpdf of the state dynamic weigths
        log_prob_weights = self.dynamic_weights_prior.log_prob(dynamic_weights_samples)
        ...
        diffusion_coefficient_diagonal = jnp.diagonal(
            x.dynamics.diffusion_coefficient, axis1=-2, axis2=-1
        )

        # Compute logpdf of the state dynamic weigths
        log_prob_diffusion_coefficient = (
            self.dynamics_diffusion_coefficient_prior.log_prob(
