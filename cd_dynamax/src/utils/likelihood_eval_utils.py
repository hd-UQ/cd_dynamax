...
from cd_dynamax.src.continuous_discrete_nonlinear_gaussian_ssm import EKFHyperParams
...
from .plotting_utils import *  # noqa: F403, F405
from .simulation_utils import generate_irregular_t_emissions
...
def experiment(
    ...
    def compute_log_likelihood_grad_and_covariances(
        new_value_1, new_value_2, hyperparams
    ):
        def marginal_loglik_fn(values):
            ...
            filtered = cdnlgssm_filter(
                new_params, emissions, times, hyperparams=hyperparams
            )
    ...
    for hyperparam_name, hyperparam in hyperparam_dict.items():
        ...
        is_psd_batched = jax.vmap(
            lambda x: jnp.all(jnp.linalg.eigvalsh(x) >= 0), in_axes=0
        )(true_pred_cov)
        ...
        def compute_single(value_1, value_2, hyperparam):
            mll, grad_value, predicted_covariances = (
                compute_log_likelihood_grad_and_covariances(
                    value_1, value_2, hyperparam
                )
            )
            is_psd_batched = jax.vmap(lambda x: jnp.all(jnp.linalg.eigvalsh(x) >= 0))(
        ...
        def compute_full(param_sweep_1, param_sweep_2, hyperparam):
            ...
            param_grid_1, param_grid_2 = jnp.meshgrid(
                param_sweep_1, param_sweep_2, indexing="ij"
            )
            ...
            results = jax.vmap(lambda v1, v2: compute_single(v1, v2, hyperparam))(
        ...
        log_likelihoods, gradients, non_psd_counts = compute_full(
            param_sweep_1, param_sweep_2, hyperparam
        )

        path_name = os.path.join(
            output_dir, f"{hyperparam_name.replace(' ', '_')}_results.npz"
        )
        np.savez_compressed(
            path_name,
...
def make_2d_plots(
    ...
    if plot_truth:
        axs[0].scatter(
            ...
            marker="X",
            s=100,
            label=f"True Parameter (LL={true_ll:.2f})",
        )
    axs[0].scatter(
        max_param_1,
        max_param_2,
        ...
        marker="o",
        s=100,
        label=f"Max Likelihood (LL={np.nanmax(lls):.2f})",
    )
    axs[0].set_xlabel("Parameter 1")
    axs[0].set_ylabel("Parameter 2")
    axs[0].set_title(
        f"Log-Likelihood Surface ({hyperparam_name}) - Original Units. True LL: {true_ll:.2f}"
    )
    axs[0].legend()
    ...
    axs[1].set_title(
        f"Non-PSD Predicted Covariances ({hyperparam_name}) - Original Units"
    )
    axs[1].legend()
    ...
    plt.savefig(
        os.path.join(
            output_dir,
            f"{hyperparam_name.replace(' ', '_')}_plots_original_units_with_gradients.png",
        )
    )
    if show:
        plt.show()
    ...
    ll_norm = matplotlib.colors.SymLogNorm(
        linthresh=1e-2, linscale=1, vmin=np.nanmin(lls), vmax=np.nanmax(lls)
    )
    ...
    axs[0].set_ylabel("Parameter 2")
    axs[0].set_title(
        f"Log-Likelihood Surface ({hyperparam_name}) - Log Scale. True LL: {true_ll:.2f}"
    )
    axs[0].legend()
    ...
    non_psd_norm = matplotlib.colors.SymLogNorm(
        linthresh=1e-2,
        linscale=1,
        vmin=np.nanmin(non_psd_counts),
        vmax=np.nanmax(non_psd_counts),
    )
    ...
    plt.savefig(
        os.path.join(
            output_dir,
            f"{hyperparam_name.replace(' ', '_')}_plots_log_scale_with_gradients.png",
        )
    )
    if show:
        plt.show()
...
def main():
    ...
    parser.add_argument(
        "--run_experiments", action="store_true", help="Run the experiments."
    )
    parser.add_argument("--plot_results", action="store_true", help="Plot the results.")
    parser.add_argument(
        "--output_dir", type=str, default=None, help="Output directory for the results."
    )
    ...
    if args.run_experiments:
        ...
        times, _, _, _, _, _ = generate_irregular_t_emissions(
            T_total=T_total,
        ...
        true_diffusion_coefficient = {
            "params": LearnableMatrix(params=state_noise_sd**2 * jnp.eye(state_dim)),
            "props": LearnableMatrix(params=ParameterProperties()),
        }
        true_emission = {
            "params": LearnableLinear(
                weights=jnp.array([[1.0, 0.0, 0.0]]), bias=jnp.zeros(emission_dim)
            ),
            "props": LearnableLinear(
                weights=ParameterProperties(), bias=ParameterProperties()
            ),
        }
        true_emission_cov = {
        ...
        states, emissions = model.sample(
            true_params, next(keys), len(times), times, transition_type="path"
        )
