...
class ParamsCDLGSSMDynamics(NamedTuple):
    ...
    bias: Union[
        ParameterProperties, Float[Array, " state_dim"], Float[Array, "ntime state_dim"]
    ]
    ...
    if init_prior is not None:
        ...
        for dict_name in sampled_params.keys():
            ...
            if params[dict_name]["params"] is None:
                ...
                print(
                    "Ignoring sampled parameters for {}, keeping user initialized values".format(
                        dict_name
                    )
                )
