...
def build_params(
    *,
    ...
    # Dynamics
    dynamics_drift_weights = (
        (-0.1 * jnp.eye(state_dim))
        if dynamics_drift_weights is None
        else dynamics_drift_weights
    )  # state-to-state
    dynamics_input_weights = (
        jnp.zeros((state_dim, input_dim))
        if dynamics_input_weights is None
        else dynamics_input_weights
    )  # input-to-state
    dynamics_bias = (
        jnp.zeros(state_dim) if dynamics_bias is None else dynamics_bias
    )  # state bias
    ...
    # Emissions
    emission_weights = (
        jnp.eye(emission_dim, state_dim)
        if emission_weights is None
        else emission_weights
    )
    emission_bias = (
        jnp.zeros(emission_dim) if emission_bias is None else emission_bias
    )  # observation bias
    emission_input_weights = (
        jnp.zeros((emission_dim, input_dim))
        if emission_input_weights is None
        else emission_input_weights
    )  # Input-to-emissions weights
    emission_cov = (
        jnp.eye(emission_dim) if emission_cov is None else emission_cov
    )  # observation cov
