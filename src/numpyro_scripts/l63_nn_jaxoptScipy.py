...
def main(**cfg):
    ...
    run = wandb.init(config=cfg, project=cfg["project"], dir=cfg["dir"], name=cfg["run_name"])
    cfg = wandb.config
    ...
    # Build "data kwargs" once so both guide.init and ELBO see the same inputs
    data_kwargs = dict(t_emissions=t_emissions, emissions=emissions_obs, **known_values)
    # Run gradient-free optimization over the ELBO
    svi_result = svi_run_grad_jaxopt(
        model,
        guide,
        data_kwargs=data_kwargs,
        init_key=next(keys),            # for guide.init
        loss_key=next(keys), # FIXED key to keep the loss deterministic
        maxfevals=cfg.num_epochs,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--opt_method", type=str, default="L-BFGS-B", choices=["Nelder-Mead", "Powell", "CG",
    ...
    # Wandb settings
    parser.add_argument("--project", type=str, default="l63_nn_scipy")
    parser.add_argument("--run_name", type=str, default=None)
    parser.add_argument("--dir", type=str, default="./wandb")  # wandb directory
    # Parse arguments
