...
def main(**cfg):
    ...
    # Using xdot =  y; ydot = mu*(1-x^2)*y - x
    # dx/dt = y
    W_true[0, idx_of(tuple([0, 1]))] = 1.0 # y
    # dy/dt = mu*y - mu*x^2*y - x
    W_true[1, idx_of(tuple([0, 1]))] = mu # y
    W_true[1, idx_of(tuple([2, 1]))] = -mu # x^2 * y
    W_true[1, idx_of(tuple([1, 0]))] = -1.0 # -x
...
if __name__ == "__main__":
    ...
    parser.add_argument("--emission_cov", type=float, default=0.1) # emission noise covariance (times identity)
    parser.add_argument("--diffusion_coeff", type=float, default=0.1)
    ...
    # Wandb settings
    parser.add_argument("--project", type=str, default="vdp_dict_laplace")
