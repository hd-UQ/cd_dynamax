...
def make_optimizer(init_lr=1e-2, 
                   factor=0.5,
                   patience=200,
                   cooldown=0,
                   clip_norm=None,
                   use_plateau=True):
    """Construct Optax optimizer for NumPyro SVI.

    Args:
        lr: base learning rate
        clip_norm: if not None, apply global norm clipping
        use_plateau: if True, wrap with reduce_on_plateau scheduler
    """
    transforms = []
    ...
    if use_plateau:
        transforms.append(reduce_on_plateau(factor=factor, patience=patience, cooldown=cooldown))

    transforms.append(optax.scale(-init_lr))  # base learning rate

    return optax_to_numpyro(optax.chain(*transforms))
...
def main(**cfg):
    ...
    optimizer = make_optimizer(
        ...
        clip_norm=cfg.clip_norm if cfg.clip_norm > 0.0 else None,
        use_plateau=cfg.use_lr_scheduler,
    )
...
if __name__ == "__main__":
    ...
    # Wandb settings
    parser.add_argument("--project", type=str, default="l63_dict_laplace_LRsched")
