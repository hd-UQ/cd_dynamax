...
- Revise MCMC - Blackjax API with latest packages
...
- UQ: priors for MLE and MAP
    - To interfere with cd-dynamax as little as possible
        - We define cd_dynamax model level [priors in utils](./src/utils/prior_utils.py)
            - the user decides what prior distribution to use, and over which specific parameters (whether they are independent, correlated, etc.)
        - Priors are defined as standalone classes, with two methods
            - log_prob:
                - input: takes params (ParamsCDLGSSM or ParamsCDNLGSSM)
                - returns: log_prob of such samples
            - sample:
                - input: key and number of samples M
                - returns: M samples within each parameter the user wants to draw from
                    - note that the definition of "parameter" here can be very complex (e.g., neural net params)
                    - But it should be consistent with the definition of the ParamsCDLGSSM or ParamsCDNLGSSM it should operate with
                    - This function will sample ONLY those the user wants to sample, the rest of the ParamsCDLGSSM or ParamsCDNLGSSM class should be handled outside
                    
        - ssm_temissions
            - log_prior: we have modified the call to use prior.log_prior if it exists
            
        - model initialization
            - if prior exists, we use it to sample the parametes of interst
            - The rest is initialized based on given values or defaults
        
        - TODO: how to deal with multiple samples?

- For latest package branch and environment
    - Why is diffeqsolve giving errors?
        - Fixed locally, PR to be done, probably fixed in new diffrax release 
...
- Robustness
    - Why does different HW impact performance so much?  
        - Can [this](https://github.com/jax-ml/jax/issues/4823) help? 
        
    - How robust is our inference/optimization/MCMC to different seeds?
...
- Remove pytrees and props -> use equinox classes

- Unified linear and nonlinear models
    - models based in equinox
    - remove the .f()

- Unified continuous and discrete state-models

- The NumPyro UI:
    - The wrapper model as the UI
        - using cd-dynamax model in the backend
            - and its configurable filter hyperparams
            
        - Sample params from NumPyro priors

    - NumPyro functions for parameter inference
        - SGD, MCMC, SVI

- Unclear:
     - replace all tf.probability distributions within cd-dynamax filtering/smoothing algorithms to:
        - NumPyro dists?
...
- Different [documents as submodules](https://gist.github.com/gitaarik/8735255) within main repo

- Bayesian System ID for irregularly-sampled, noisy data: benefits of a differentiable, marginal loglikelihood-based approach
...
- Bayesian System ID in reduced parametric spaces:
    - a prior averaged FIM based reduction

- Dimensionality reduction (canonical forms) of nonlinear system

- Empirical Bayes and Hierarchical learning 

- Prior predictive
    - Eliezer
    
- Model error learning with UQ
    - NNs
...
- Beyond classic filtering algorithms
    - Implement autodiff SMC
        - Dan @ Basis might do this

    - Incoporate Optimal Transport ideas to filtering?
    
    - Linearization of KL divergences 
    
- Multilevel/multi-fidelity solvers and estimations of loglikelihood
...
- [Scott Linderman](https://web.stanford.edu/~swl1/)
    ...
    - interested in our work
        - he has people in his lab working on similar topics
    - April 28th, 2025 meeting
        - let us move towards 2.0 together
        - summer interns
        - incorporate continuous discrete machinery

- [Niall M. Mangan](http://www.niallmangan.com/)
    - Knows Sindy literature
    - Interested in our work, maybe applications

- [Jeroen S.W. Lamb](https://scholar.google.com/citations?user=gpxgxPIAAAAJ&hl=en)
    - Different perspective on parameterizations of linear systems?

- [Boumediene Hamzi](https://scholar.google.com/citations?user=2goK6M0AAAAJ&hl=en)
    - Matt knows him, interested in learning reduced/normal forms for partially observed nonlinear systems (equivalence classes)

- [George](https://scholar.google.com/citations?user=CKYZLxYAAAAJ&hl=en)
    - However, once implemented, smoothers/filters for non-linear systems employ various approximations.
        - Crucially, these approximations may not be the same, and thus their overall approximated likelihoods may be different.
        - Do these differences matter? When?
    - A simple test-bed:
        - Take a linear-gaussian system
        - Both the Kalman Filter and Kalman Smoother should give identical (and exactly correct) marginal log-likelihood for parameters
        - Ensemble Kalman Filters/Smoothers should also give exactly correct MLL in infinite particle limit. But, how do they compare in finite particle regime?
        - Then, could go to a non-linear system and see if there is a gap there too.

    - how does (1) annealing or (2) neural network reparameterization improve the optimization landscape of dynamical system fitting.
    
- Nadège Polette (CEA/DAM, GEOSCIENCES), Olivier Le Maître (PLATON, CMAP), Pierre Sochala (CEA/DAM), Alexandrine Gesret (GEOSCIENCES)
    - [Change of Measure for Bayesian Field Inversion with Hierarchical Hyperparameters Sampling](https://arxiv.org/abs/2404.12688v2)
    - Hierarchical Bayesian Framework to infer GPs with KL approximation along with efficient hyperparameter inference
