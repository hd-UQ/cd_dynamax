...
for dynamics_approx_order in [1.0, 2.0]:
    ...
    for state_order in ["first", "second"]:
        ...
        print(
            f"Fitting continuous-discrete non-linear model with SGD + {state_order}-order EKF"
        )
        ...
        print(
            f"Continuous-Discrete time non-linear {state_order}-order EKF filtering: post-fit"
        )
...
print("**********************************")
print(
    "Running Unscented Kalman Filter with non-linear model class and data from first-order CDNLGSSM model"
)
...
for N_particles in [1e2, 1e3, 1e4]:
    for perturb_measurements in [True]:
        ...
        print(
            f"Running Ensemble Kalman Filter (perturb_measurements={perturb_measurements}, N_particles={N_particles}) with non-linear model class and data from first-order CDNLGSSM model"
        )
