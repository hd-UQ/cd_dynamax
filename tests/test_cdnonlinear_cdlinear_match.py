...
from cd_dynamax.src.utils.experiment_utils import generate_t_emissions
    ...
    cdnlgssm_params, cdnlgssm_props = cdnlgssm_model.initialize(
        ...
        dynamics_diffusion_coefficient={
            "params": LearnableMatrix(
                params=cdlgssm_params.dynamics.diffusion_coefficient
            ),
        ...
        emission_function={
            "params": LearnableLinear(
                weights=cdlgssm_params.emissions.weights,
                bias=cdlgssm_params.emissions.bias,
            ),
            "props": LearnableLinear(
                weights=ParameterProperties(),
                bias=ParameterProperties(trainable=False),  # We do not learn bias term!
    ...
    cdnlgssm_params, cdnlgssm_props = cdnlgssm_model.initialize(
        ...
        emission_distribution={
            "params": LearnableGaussianEmission(
                ...
                emission_cov=LearnableMatrix(
                    params=0.1 * jnp.eye(cdlgssm_model.emission_dim)
                ),
            ),
...
def test_cdnonlinear_filter_cdlinear_kf_match(seed):
    ...
    cdlgssm_model, cdlgssm_params, cdlgssm_props, cdlgssm_key = init_cdlgssm_model(
        next(keys)
    )
    ...
    for dynamics_approx_order in [1.0, 2.0]:
        ...
        print(
            f"... all filtering (with forecasting and emissions) tests passed for CD-NLGSSM model with dynamics_approx_order={dynamics_approx_order}"
        )
    ...
    print(
        "... all filtering (with forecasting and emissions) tests passed for CD-NLSSM model"
    )
...
def check_nonlinear_linear_filter_match(
    ...
    if isinstance(cddynamax_params, ParamsCDNLGSSM):
        ...
        filter_hyperparams_list = [DPFHyperParams(N_particles=1000)]
    else:
    ...
    for filter_hyperparams in filter_hyperparams_list:
        print(
            f"Testing filtering for nonlinear model with filter {type(filter_hyperparams)}..."
        )
        ...
        assert jnp.isfinite(marginal_loglik), "Marginal log-likelihood is not finite"
        ...
        assert forecasted.forecasted_state_means.shape == (
            len(t_forecast_emissions),
            cddynamax_model.state_dim,
        ), (
        ...
        assert forecasted.forecasted_state_covariances.shape == (
            len(t_forecast_emissions),
            cddynamax_model.state_dim,
            cddynamax_model.state_dim,
        ), (
        ...
        assert forecasted_emissions["mean"].shape == (
            len(t_forecast_emissions),
            cddynamax_model.emission_dim,
        ), (
        ...
        assert forecasted_emissions["cov"].shape == (
            len(t_forecast_emissions),
            cddynamax_model.emission_dim,
            cddynamax_model.emission_dim,
        ), (
        ...
        print(
            "\tChecking cd-dynamax nonlinear model filtered emissions match original cdlgssm emissions..."
        )
        compare_structs(
            filtered_emissions, cdlgssm_filtered_emissions, accept_failure=True
        )
        print(
            "\tChecking cd-dynamax nonlinear model forecasted emissions match original cdlgssm emissions..."
        )
        compare_structs(
            forecasted_emissions, cdlgssm_forecasted_emissions, accept_failure=True
        )
...
def test_cdnonlinear_smoother_cdlinear_ks_match(seed):
    ...
    for dynamics_approx_order in [1.0, 2.0]:
        ...
        print(
            f"... all smoothing tests passed for CD-NLGSSM model with dynamics_approx_order={dynamics_approx_order}"
        )
...
def check_nonlinear_linear_smoother_match(
    ...
    if isinstance(cddynamax_params, ParamsCDNLGSSM):
        ...
        raise ValueError(
            f"Unknown model parameters type {type(cddynamax_params)} for smoothing check"
        )
    ...
    for filter_hyperparams in filter_hyperparams_list:
        ...
        print(
            f"Testing smoothing for nonlinear model with filter {type(filter_hyperparams)}..."
        )
