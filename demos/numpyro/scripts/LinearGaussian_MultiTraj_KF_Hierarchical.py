...
from cd_dynamax import ContDiscreteLinearGaussianSSM, make_key_sequence, make_optimizer
...
def main(cfg):
    ...
    def stabliize(A):  # (keeping your original name)
        ...
        A_stable = jnp.where(
            spectral_norm > cfg.max_spectral_norm,
    ...
    def model(t_emissions, emissions=None, **kwargs):
        ...
        weights = get_or_sample("weights", dist_weights, kwargs.get("weights"))
        ...
        dist_mu = dist.TruncatedNormal(
            loc=jnp.full((state_dim,), cfg.m_mu),
        ...
        dist_sigma = dist.TruncatedNormal(
            loc=jnp.full((state_dim,), cfg.m_sigma),
        ...
        dist_biases = dist.TruncatedNormal(
            loc=jnp.broadcast_to(mu, (N, state_dim)),
        ...
        cdlgssm = ContDiscreteLinearGaussianSSM(
        ...
        if emissions is None:
            ...
            def _sample_one(k, b_vec):
                ...
                states, ems = cdlgssm.sample(
                    params=params_i, num_timesteps=T, key=k, t_emissions=t_emissions
                )
                return states, ems
        ...
        def _filter_one(ems_i, b_vec):
            params_i = _build_params_for_bias(b_vec)
            out = cdlgssm.filter(
                params=params_i, emissions=ems_i, t_emissions=t_emissions
            )
    ...
    t_emissions = sample_t_emissions(
        start=0.0, stop=cfg.T, dt=cfg.dt, regular=cfg.t_regular, key=next(keys)
    ).reshape(-1, 1)
    ...
    if cfg.N_trajectories > 5:
        print(
            f"Plotting the first {n_plotted_traj} of {cfg.N_trajectories} trajectories..."
        )
    for i in range(n_plotted_traj):
        fig = plot_simulated_data(
            t=t_emissions, states=states_true[i], emissions=emissions_obs[i]
        )
    ...
    known_values = {
        key: value for key, value in true_values.items() if key not in cfg.learnables
    }
    ...
    if "mu" in learned:
        ...
        plt.close(fig)
    if "sigma" in learned:
        fig = plot_coeff_heatmaps(
            sigma_true.reshape(-1, 1), learned["sigma"].reshape(-1, 1)
        )
    ...
    fig = plot_param_recovery(
        true_values, guide=guide, svi_result=svi_result, groups=scatter_groups
    )
    ...
    learned_data = predictive_learned(
        next(keys), t_emissions=t_emissions, emissions=emissions_obs, **known_values
    )
    for name in neg_log_names:
        print(
    ...
    fig = plot_traj_kde(
        {
            "Observed traj": long_true_data["emissions"].squeeze(0)[
                idx_traj, burnin_idx:
            ],
            "Long learned traj": long_learned_data["emissions"].squeeze(0)[
                idx_traj, burnin_idx:
            ],
        }
    )
    ...
    fig = plot_traj_kde(
        {
            ...
            "Learned states": long_learned_data["states"].squeeze(0)[
                idx_traj, burnin_idx:
            ],
        }
    )
    ...
    if cfg.run_mcmc:
        ...
        mcmc = MCMC(
            nuts_kernel,
            num_warmup=cfg.nuts_warmup,
            num_samples=cfg.nuts_samples,
            num_chains=1,
        )
        mcmc.run(
            next(keys), t_emissions=t_emissions, emissions=emissions_obs, **known_values
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--svi_guide",
        type=str,
        default="AutoMultivariateNormal",
        choices=[
            "AutoDelta",
            "AutoDiagonalNormal",
    ...
    # Output settings
    parser.add_argument(
        "--dir", type=str, default="demo_outputs/LinearGaussian_MultiTraj_KF_HierTN"
    )
