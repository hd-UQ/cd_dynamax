...
def main(cfg):
    ...
    def model(t_emissions, emissions=None, **kwargs):
        if "drift" in kwargs and kwargs["drift"] is not None:
            ...
            drift = lambda x: adjust_rhs(x, drift_base(x))
    ...
    known_values = {
        key: value for key, value in true_values.items() if key not in ["drift"]
    }
    ...
    long_t_emissions = jnp.arange(start=0.0, stop=100 * cfg.T, step=cfg.dt).reshape(
        -1, 1
    )
...
if __name__ == "__main__":
    ...
    # Prior parameters
    parser.add_argument(
        "--hidden_dims", type=int, nargs="+", default=[100]
    )  # hidden layer sizes for the NN drift
    ...
    parser.add_argument(
        "--t_regular", type=int, default=1
    )  # 1 for True (regular sampling), 0 for False (uniform random sampling)
    parser.add_argument(
