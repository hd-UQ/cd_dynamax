...
def main(cfg):
    ...
    # NumPyro model
    def model(
        t_emissions, emissions=None, store_filtered=False, store_grad=False, **kwargs
    ):
        ...
        if store_filtered:
            ...
            numpyro.deterministic(
                "neg_loglik_steps", -filtered.posterior_extras["loglik_step"]
            )
            ...
            numpyro.deterministic(
                "x_ens_filtered", filtered.posterior_extras["x_ens_filtered"]
            )
            numpyro.deterministic(
                "x_ens_predicted", filtered.posterior_extras["x_ens_predicted"]
            )
    ...
    print(
        "Checking predictive/ filtering performance at true parameter + small noise..."
    )
    ...
    perturbed_data = perturbed_truth_predictive(
        next(keys),
        t_emissions=t_emissions,
    ...
    for name in neg_log_names:
        print(
            f"Perturbed model's {name} from filtering: {float(perturbed_data[name].item())}"
        )

    # Log the trajectory from filtering
    if cfg.log_filtering:
        print(
            f"True+epsilon (perturbed) model's neg_log_likelihood from filtering: {float(perturbed_data['neg_log_likelihood'].item())}"
        )
        fig = plot_particle_diagnostics(
            t_emissions=t_emissions.squeeze(),
    ...
    if nan_fraction > 0.0:
        print(
            f"Warning: {nan_fraction:.2%} of prior predictive neg_log_likelihood samples are NaN"
        )
        ...
        raise ValueError(
            f"NaN values found in prior predictive neg_log_likelihood: {nan_fraction:.2%} of samples are NaN. Training is TOO DANGEROUS to proceed; please refine your prior, protect the RHS from large jumps, ensure covariances are PSD, and/or try 64bit precision."
        )
    ...
    if nan_fraction_grad > 0.0:
        print(
            f"Warning: {nan_fraction_grad:.2%} of prior predictive grad_neg_log_likelihood_w samples are NaN"
        )
        # Show any one example weights matrix with NaN values
        ...
        # Optionally, you can raise an error here to stop execution
        raise ValueError(
            f"NaN values found in prior predictive grad_neg_log_likelihood_w: {nan_fraction_grad:.2%} of samples are NaN. Training is TOO DANGEROUS to proceed; please refine your prior, protect the RHS from large jumps, ensure covariances are PSD, and/or try 64bit precision."
        )
    ...
    print(
        f"Initialization neg_log_likelihood from filtering: {float(init_data['neg_log_likelihood'].item())}"
    )
    if jnp.isnan(init_data["neg_log_likelihood"]):
        print("Initialization weights: ", W_init)
        raise ValueError(
            "NaN value found in initialization neg_log_likelihood. Training is TOO DANGEROUS to proceed; please refine your prior, protect the RHS from large jumps, ensure covariances are PSD, and/or try 64bit precision."
        )
    ...
    print(
        f"Initialization grad_neg_log_likelihood_w from filtering: {init_data['grad_neg_log_likelihood_w']}"
    )
    if jnp.any(jnp.isnan(init_data["grad_neg_log_likelihood_w"])):
        print("Initialization weights: ", W_init)
        raise ValueError(
            "NaN value found in initialization grad_neg_log_likelihood_w. Training is TOO DANGEROUS to proceed; please refine your prior, protect the RHS from large jumps, ensure covariances are PSD, and/or try 64bit precision."
        )
    ...
    fig = plot_traj_kde(
        {
            ...
            "Long learned observations": long_learned_data["emissions"].squeeze(0)[
                burnin_idx:
            ],
        },
        per_dim=False,
    ...
    fig = plot_traj_kde(
        {
            ...
            "Long learned states": long_learned_data["states"].squeeze(0)[burnin_idx:],
        },
        per_dim=False,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--eps_perturb", type=float, default=0.1
    )  # noise level for initializing near the truth

    # True system parameters
    parser.add_argument(
        "--initial_cov", type=float, default=25.0
    )  # initial state covariance (times identity)
    parser.add_argument(
        "--emission_cov", type=float, default=1.0
    )  # emission noise covariance (times identity)
    ...
    parser.add_argument(
        "--state_dim", type=int, default=5
    )  # number of states in the Lorenz96 model
    parser.add_argument(
        "--emission_dim", type=int, default=5
