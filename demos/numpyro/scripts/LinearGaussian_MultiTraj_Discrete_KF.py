...
from cd_dynamax import LinearGaussianSSM, make_key_sequence, make_optimizer
...
def main(cfg):
    ...
    def model(t_emissions, emissions=None, **kwargs):
        ...
        if "emission_sd" in cfg.learnables:
            lp = lp + dist.Uniform(cfg.emission_sd_low, cfg.emission_sd_high).log_prob(
                emission_sd
            )
        if "diffusion_coeff" in cfg.learnables:
            lp = lp + dist.Uniform(cfg.diffusion_low, cfg.diffusion_high).log_prob(
                diffusion_coeff
            )

        # Build the model and its parameters
        lgssm = LinearGaussianSSM(
            state_dim=state_dim, emission_dim=emission_dim, input_dim=0
        )
    ...
    # bias ~ U(b_low, b_high)
    bias_true = jr.uniform(
        next(keys), shape=(state_dim,), minval=cfg.b_low, maxval=cfg.b_high
    )
    ...
    sim_data = Predictive(model, num_samples=1)(
        next(keys), t_emissions=t_emissions, **true_values
    )
    ...
    for i in range(n_plotted_traj):
        fig = plot_simulated_data(
            t=t_emissions,
            states=sim_data["states"].squeeze(0)[i],
            emissions=emissions_obs[i],
        )
    ...
    if cfg.run_mcmc:
        ...
        fig = plot_param_recovery(
            true_values, mcmc=mcmc, groups=scatter_groups, title="MCMC: learned vs true"
        )
...
if __name__ == "__main__":
    ...
    # Output settings
    parser.add_argument(
        "--dir", type=str, default="demo_outputs/LinearGaussian_MultiTraj_Discrete_KF"
    )
