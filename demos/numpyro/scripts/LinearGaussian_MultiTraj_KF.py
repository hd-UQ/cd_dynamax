...
def main(cfg):
    ...
    def model(t_emissions, emissions=None, **kwargs):
        ...
        if emissions is None:
            ...
            def _sample_one(k):
                states, ems = cdlgssm.sample(
                    params=params, num_timesteps=T, key=k, t_emissions=t_emissions
                )
        ...
        def _filter_one(ems_i):
            out = cdlgssm.filter(
                params=params, emissions=ems_i, t_emissions=t_emissions
            )
...
if __name__ == "__main__":
    ...
    # Output settings
    parser.add_argument(
        "--dir", type=str, default="demo_outputs/LinearGaussian_MultiTraj_KF"
    )
