...
def plot_filter_then_forecast(
    ...
    for ftf_key in ftf_keys if isinstance(ftf_keys, (list)) else [ftf_keys]:
        print(
            f"Loading {filter_info['name']} filtering from T={T0} up to T={T_filter_end} and forecasting up to T={T_forecast_end} (with ftf_key={ftf_key}) results..."
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Plot the filter then forecast experiment with specified configurations."
    )
    parser.add_argument(
        "--config_path",
        type=str,
