...
def fit_model_to_data(
    ...
    results_dir = os.path.join(
        results_dir, os.path.splitext(os.path.basename(fit_config_file))[0]
    )
    ...
    if not os.path.exists(fit_config_filepath):
        raise FileNotFoundError(
            f"Configuration file '{fit_config_filepath}' not found."
        )
    ...
    if "sgd" in optim_configs:
        ...
        for idx, key in enumerate(fit_keys):
            print(
                f"Running SGD MAP estimation with fit_key: {key} (Index {idx + 1} of {len(fit_keys)})"
            )
            ...
            with open(
                os.path.join(results_dir, f"sgd_model_fit_fitkey{key}.pkl"), "wb"
            ) as f:
    ...
    if "mcmc" in optim_configs:
        ...
        for idx, key in enumerate(fit_keys):
            ...
            print(
                "Using fit results from SGD for MCMC initialization."
                if "sgd" in optim_configs
                else "No previous SGD fit; using initial parameters for MCMC initialization."
            )
            print(
                f"Running MCMC MAP estimation with fit_key: {key} (Index {idx + 1} of {len(fit_keys)})"
            )
            mcmc_result = model.fit_mcmc(
                initial_params=sgd_results["params_fitted"]
                if "sgd" in optim_configs
                else params,
                props=props,
                emissions=data["emissions"],
            ...
            save_results_file = os.path.join(
                results_dir,
                mcmc_config.get("type", "unknown") + f"_mcmc_model_fit_fitkey{key}.pkl",
            )
    ...
    if "scipy" in optim_configs or "scipy_jaxopt" in optim_configs:
        ...
        for idx, key in enumerate(fit_keys):
            print(
                f"Running {config_key} optimization MAP estimation with fit_key: {key} (Index {idx + 1} of {len(fit_keys)})"
            )

            return_param_history = scipy_config.getboolean(
            ...
            print(
                f"{config_key} optimization MAP estimation complete for fit_key: {key}"
            )
            ...
            # Save the results
            with open(
                os.path.join(results_dir, f"scipy_model_fit_fitkey{key}.pkl"), "wb"
            ) as f:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fit model to data, according to specified configurations of data, model and filtering."
    )
    parser.add_argument(
        "--config_path",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="results/fit_model_to_data",
