...
def compare_filter_then_forecast(
    ...
    if not os.path.exists(results_dir):
        ...
        print(
            "Please run the filtering and forecasting experiment first to generate results."
        )
        import pdb
    ...
    for ftf_key in ftf_keys if isinstance(ftf_keys, (list)) else [ftf_keys]:
        ...
        for filter_idx, filter_config_file in enumerate(filter_config_files):
            ...
            filter_results_dir = os.path.join(
                results_dir, os.path.basename(filter_config_file).split(".")[0]
            )
            if not os.path.exists(filter_results_dir):
                print(
                    "Warning: Filter results directory does not exist at:",
                    filter_results_dir,
                )
                print(
                    "Please run the filtering and forecasting experiment first to generate results for this filter."
                )
                continue

            print(
                f"Loading {all_filter_info[filter_idx]['name']} filtering from T={T0} up to T={T_filter_end} and forecasting up to T={T_forecast_end} (with ftf_key={ftf_key}) results..."
            )
            # Load the results
            filter_results_file = os.path.join(
                filter_results_dir, "results_ftfkey{}.pkl".format(ftf_key)
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Compare the filter then forecast experiment with a set of filter configurations."
    )
    parser.add_argument(
        "--config_path",
        type=str,
    ...
    if len(args.filter_config_file) == 1:
        ...
        if filter_config_files[0].lower() == "all":
            ...
            raise ValueError(
                "We need more than one filter_config_file argument for comparison."
            )
    else:
        filter_config_files = args.filter_config_file
