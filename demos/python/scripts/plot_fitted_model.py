...
def plot_fitted_model_to_data(
    ...
    learnable_model, learnable_params, learnable_props = (
        create_cddynamax_model_from_config(
            config_path=config_path,
    ...
    if "sgd" in optim_configs:
        ...
        for idx, key in enumerate(fit_keys):
            print(
                f"Loading SGD MAP estimation results with fit_key: {key} (Index {idx + 1} of {len(fit_keys)})"
            )

            # Load the results, for each fit_key separately
            with open(
                os.path.join(results_dir, f"sgd_model_fit_fitkey{key}.pkl"), "rb"
            ) as f:
            ...
            plot_mll_learning_curve(
                ...
                plot_save_path=os.path.join(
                    plotting_dir, f"mll_learning_curve_fitkey{key}"
                ),
            )
            ...
            plot_param_sequences(
                ...
                pairwise_plots=True,
                plot_save_path=os.path.join(
                    plotting_dir, f"parameter_trajectories_fitkey{key}"
                ),
            )
    ...
    if "mcmc" in optim_configs:
        ...
        for idx, key in enumerate(fit_keys):
            print(
                f"Loading MCMC MAP estimation results with fit_key: {key} (Index {idx + 1} of {len(fit_keys)})"
            )
            ...
            plot_param_dist(
                ...
                pairwise_plots=True,
                plot_save_path=os.path.join(
                    plotting_dir, f"parameter_distributions_fitkey{key}"
                ),
            )
    ...
    if "scipy" in optim_configs or "scipy_jaxopt" in optim_configs:
        ...
        for idx, key in enumerate(fit_keys):
            print(
                f"Loading Scipy MAP estimation results with fit_key: {key} (Index {idx + 1} of {len(fit_keys)})"
            )

            # Load the results, for each fit_key separately
            load_results_file = os.path.join(
                results_dir, f"scipy_model_fit_fitkey{key}.pkl"
            )
            with open(load_results_file, "rb") as f:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Plot fitted model to data --- as run by fit_model_to_data.py according to specified configurations of data, model and filtering."
    )
    parser.add_argument(
        "--config_path",
        type=str,
