...
def run_filter_then_forecast(
    ...
    # Set the key for filtering and forecasting
    ftf_key = (data_key + 10) if ftf_key is None else ftf_key  # i.e. 10 more than data_key or given ftf_key
    ...
    # Generate emissions means/covs from predicted states
    emissions_mean, emissions_cov = cdnlgssm_emissions(
        params=params,
        t_states=data['t_emissions'][:stop_idx_filter],
...
if __name__ == "__main__":
    ...
    # Set-up overrides dictionary with data_key if provided
    overrides = {}
    if args.data_key is not None:
        overrides={
            'data_generation.key': args.data_key,
        }

    if args.filter_config_file.lower() != "all":
        print("Running filtering and forecasting experiment...")
        # Single experiment with specified filter configuration
        ...
        print(f"Running filtering and forecasting experiments for all filters")
